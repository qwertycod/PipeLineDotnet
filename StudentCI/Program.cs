...
internal class Program
{
   ...
   // private const string CONNECTION_STRING = "Host=localhost:5455;" + "Username=postgres;" + "Password=example;" + "Database=my_db";
    ...
    private static void Main(string[] args)
    {
        ...
        static void m1(string conn)
        {
            using var cmd = new NpgsqlCommand();
            using (NpgsqlConnection connection = new NpgsqlConnection(connectionString: conn))
            {
                connection.Open();
                cmd.CommandText = $"DROP TABLE IF EXISTS birds";
        ...
        static string GetConnectionString(string postgreSqlConnectionString)
        {
            ...
            string dbName = "apple_db"; //  connBuilder.Database;

            var masterConnection = postgreSqlConnectionString;

            using (NpgsqlConnection connection = new NpgsqlConnection(masterConnection))
            {
                connection.Open();
            ...
            //  postgreSqlConnectionString = masterConnection.Replace("postgres1", dbName);
            ...
            //var masterConnection = postgreSqlConnectionString.Replace(dbName, "postgres1");
            ...
            //    //var checkTableCommand = "SELECT EXISTS (SELECT EXISTS ( SELECT 1 FROM pg_tables WHERE tablename = 'Products') AS table_existence";

            //    //NpgsqlCommand sqlCmd = null;

            //    //try
            //    //{
            //    //    sqlCmd = new NpgsqlCommand(checkTableCommand, connection);
            //    //    var res = sqlCmd.ExecuteScalar();

            //    //    //if (res.ToString() == "f")
            //    //    //{
            //    //        using var command1 = new NpgsqlCommand($"CREATE TABLE Apple ( Id serial PRIMARY KEY, Name VARCHAR ( 50 )   NULL, Category VARCHAR ( 50 )  NULL, Price integer NULL )", connection);
            //    //        command1.ExecuteNonQuery();
            //    //    //}
            //    //}
            //    //catch (Exception ex)
